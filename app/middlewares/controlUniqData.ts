...
export const controlUniqData = {
  ...
  async uniqueDataControlEstate(
    ...
    const dataToControl = await dataSource
      .getRepository(Estate)
      .find({ where: { name: dataRequest.name } });
   ...
   * @returns
   */
  async uniqueDataControlLocation(
    req: Request,
    res: Response,
    next: NextFunction,
  ) {
    const dataRequest = req.body;
    console.log(dataRequest,'LOCATION');

    if (
      dataRequest.num === undefined ||
   ...
   * @returns
   */
  async uniqueDataControlCustomer(
    req: Request,
    res: Response,
    next: NextFunction,
  ) {
    ...
    if (
      dataRequest.firstname === undefined ||
      dataRequest.lastname === undefined ||
      dataRequest.tel === undefined ||
      dataRequest.type_of_customer === undefined ||
   ...
   * @returns
   */
  async uniqueDataControlManager(
    req: Request,
    res: Response,
    next: NextFunction,
  ) {
    ...
    const dataToControl = await dataSource
      ...
      .find({ where: { email: dataRequest.email, login: dataRequest.login } });
   ...
   * @returns
   */
  async uniqueDataControlParking(
    req: Request,
    res: Response,
    next: NextFunction,
  ) {
    ...
    if (dataRequest.name === undefined || dataRequest.price === undefined) {
      return res.status(400).json({ Error: 'Formulaire non complet' });
    }
